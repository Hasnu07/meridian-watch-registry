...
// Resolve the owning user for any entity we can be handed. We accept a
// profile (has owner_id), a watch (has owner_id OR profile_id), or fall
// back to chasing IDs through the DB.
function resolveOwnerId(entity) {
  if (!entity) return null;
  if (entity.owner_id) return entity.owner_id;
  if (entity.profile_id) return db.getOwnerIdForProfile(entity.profile_id);
  return null;
}

async function send(message, entity) {
  try {
    const ownerId = resolveOwnerId(entity);
    if (!ownerId) return;

    const apiUrl     = db.getSetting(ownerId, 'greenapi_api_url');
    const instanceId = db.getSetting(ownerId, 'greenapi_instance_id');
    const apiToken   = db.getSetting(ownerId, 'greenapi_api_token');
    const groupId    = db.getSetting(ownerId, 'greenapi_group_id');
    if (!instanceId || !apiToken || !groupId) return; // owner hasn't configured WhatsApp
